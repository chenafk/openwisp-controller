...
function initCommandDropdown($) {
  ...
  $(function () {
    ...
    Object.keys(schema).forEach(function (el) {
      owCommandBtns += `<button class="ow-command-btn option" data-command="${el}">${schema[el].title}</button>`;
    });
    widgetElement = `
            <li>
                ...
                <div id="ow-command-confirm-dialog-wrapper" class="ow-hide">
                    <div id="ow-command-confirm-dialog">
                        ...
                        <div id="ow-command-confirm-btn-wrapper">
                            <button class="button" id="ow-command-confirm-yes">${gettext(
                              "Yes",
                            )}</button>
                            <button class="button" id="ow-command-confirm-no">${gettext(
                              "No",
                            )}</button>
                        </div>
                    </div>
  ...
  function updateRecentCommands($, response) {
    ...
    function getElement(response, counter) {
      ...
      return `<div class="inline-related has_original dynamic-command_set-2" id="command_set-2-${counter}">
                <h3><b>Recent Commands:</b>&nbsp;<span class="inline_label">«${
                  response.type
                }» ${sentOn} ${dateTimeStampToDateTimeLocaleString(
                  new Date(response.created),
                )}</span></h3>
                    <fieldset class="module aligned ">
                    <div class="form-row field-status">
                        <div><label>Status:</label><div class="readonly">${
                          response.status
                        }</div></div>
                    </div>
                    <div class="form-row field-type">
                        <div><label>Type:</label><div class="readonly">${
                          response.type
                        }</div></div>
                    </div>
                    ...
                    <div class="form-row field-created">
                        <div><label>Created:</label> <div class="readonly">${getFormattedDateTimeString(
                          response.created,
                        )}</div></div>
                    </div>
                ...
                <input type="hidden" name="command_set-2-${counter}-id" value="${
                  response.id
                }" id="id_command_set-2-${counter}-id">
                <input type="hidden" name="command_set-2-${counter}-device" value="${
                  response.device
                }" id="id_command_set-2-${counter}-device">
            </div>`;
    }
  }
...
function getCommandApiUrl() {
  return `${owControllerApiHost.origin}${owCommandApiEndpoint.replace(
...
function dateTimeStampToDateTimeLocaleString(dateTimeStamp) {
  let userLanguage = navigator.language || navigator.userLanguage,
    date = dateTimeStamp.toLocaleDateString(userLanguage, {
      day: "numeric",
      month: "short",
      year: "numeric",
    }),
